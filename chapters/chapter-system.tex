...
\chapter{Our method}
\label{sec:pipeline}
In this chapter we explain how we build our network from the \gls{resnet}, under which conditions we train the networks \tref{sec:pipeline:training} and how we generate bounding box object predictions on top of the pixelwise class probabilities \tref{sec:pipeline:eval}.
...
We preprocess the training samples with only one step, by subtracting the mean of all images of the dataset from each sample \citep{krizhevsky_imagenet_2012}. Through this step the input data becomes zero-centered.\\
...
Our desired output of the algorithm is one or are multiple boxes enclosing each a section of the image, which are predicted to contain the searched for type of object. The \gls{fcn} returns a score map as output. Each score predicts the probability of this pixel to belong to the class we trained for. Therefore we need a method to return high scoring bounding boxes from a probability map. The here used way of doing this consists of multiple steps:\\
\begin{my_list_num}
...
\end{figure}
A lot of images produce probability maps that are sparsely filled. We want to penalize these uniform negative regions. Positive regions receive a positive score from the network, while not-positive regions are at-most zero. Here we introduce a distance transform to score regions of near-zero values \fref{fig:distance_transform}.\\
...
\end{equation}
The transformed image is normalized and substracted from the original probability map. This additional step showed to increases mean \gls{auc} on \textsc{Pascal}-Part by 2\% for set sizes $< 25$.
