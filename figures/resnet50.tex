...
\begin{figure}
    ...
    \begin{tikzpicture}[font=\scriptsize]
        {[start chain, node distance=0.3cm]
           \node[fill=orange!20,on chain,draw] (RB1) at (2.3,0) {\resblock{256}{64}};
           \node[below=0.8cm] at (RB1) {$\times$ 3};
           \node[fill=orange!20,above=1cm,draw,minimum width=2cm] at (RB1) {$56\times56$};

           \node[fill=green!20,on chain,draw,join=by {->}] (RB2) {\resblock{512}{128}};
           \node[below=0.8cm] at (RB2) {$\times$ 4};
           \node[fill=green!20,above=1cm,draw,minimum width=2cm] at (RB2) {$28\times28$};

           \node[fill=blue!20,on chain,draw,join=by {->}] (RB3) {\resblock{1024}{256}};
           \node[below=0.8cm] at (RB3) {$\times$ 6};
           \node[fill=blue!20,above=1cm,draw,minimum width=2cm] at (RB3) {$14\times14$};

           \node[fill=pink!20,on chain,draw,join=by {->}] (RB4) {\resblock{2048}{512}};
           \node[below=0.8cm] at (RB4) {$\times$ 3};
           \node[fill=pink!20,above=1cm,draw,minimum width=2cm] at (RB4) {$7\times7$};
        }
